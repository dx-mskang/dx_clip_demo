#!/bin/bash

echo "1: Single Channel Demo"
echo "1-2: Single Channel Demo (Terminal Mode)"
echo "1-3: Single Channel Demo (Camera Mode & Terminal Mode)"
echo "2: Multi Channel Demo"
echo "2-2: Multi Channel Demo (Terminal Mode)"
echo "2-3: Multi Channel Demo (Camera Mode & Terminal Mode)"
echo "0: Default Demo"

read -t 10 -p "which AI demo do you want to run:(timeout:10s, default:0)" select

case $select in
        1)./run_pia_demo_pyqt.sh --number_of_channels 1;;
        1-2)./run_pia_demo_pyqt.sh --number_of_channels 1 --terminal_mode 1;;
        1-3)./run_pia_demo_pyqt.sh --number_of_channels 1 --terminal_mode 1 --camera_mode 1;;
        2)./run_pia_demo_pyqt.sh --number_of_channels 16;;
        2-2)./run_pia_demo_pyqt.sh --number_of_channels 16 --ternimal_mode 1;;
        2-3)./run_pia_demo_pyqt.sh --number_of_channels 16 --terminal_mode 1 --camera_mode 1;;
        *)./run_pia_demo_pyqt.sh;;
esac